...
// ── buildHolidayMapServer ─────────────────────────────────────────────────────
// Wraps buildHolidayMap and merges in any real dates fetched from online sources
// and stored in app_preferences as 'holiday_overrides'.
// Pass a db instance to enable override merging; omit for pure calculated output.
function buildHolidayMapServer(year, enabledSetIds, db) {
  const map = buildHolidayMap(year, enabledSetIds)
  if (db) {
    try {
      const row = db.prepare("SELECT value FROM app_preferences WHERE key = 'holiday_overrides'").get()
      const overrides = row ? JSON.parse(row.value) : {}
      const prefix = String(year)
      for (const [dateStr, entries] of Object.entries(overrides)) {
        if (!dateStr.startsWith(prefix)) continue
        if (!map[dateStr]) map[dateStr] = []
        for (const e of entries) {
          // Avoid duplicates by name
          if (!map[dateStr].find(x => x.name === e.name)) {
            map[dateStr].push(e)
          }
        }
      }
    } catch (_) { /* DB not available — fall through to calculated only */ }
  }
  return map
}

module.exports = { buildHolidayMapServer }
